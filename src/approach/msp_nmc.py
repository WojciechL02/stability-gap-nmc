...
class Appr(Inc_Learning_Appr):
    ...
    def train_epoch(self, t, trn_loader):
        ...
        if t > 0:
            ...
            current_classes_prototypes = self.exemplar_means[self.model.task_offset[-1]:]
            self.exemplar_means = []
            self.compute_mean_of_exemplars(trn_loader, self.val_loader_transform)
            if self.update_prototypes:
                # update new classes prototypes
                self.compute_means_of_current_classes(trn_loader)
            else:
                # leave prototypes of current classes unchanged
                self.exemplar_means.extend(current_classes_prototypes)
