...
class Logger(ExperimentLogger):
    ...
    def log_scalar(self, task, iter, name, value, group=None, curtime=None):
        if task is None:
            tag = "{}_{}".format(group, name)
        else:
            tag = "t{}/{}_{}".format(task, group, name)
        self.tbwriter.add_scalar(tag=tag, scalar_value=value, global_step=iter)
    ...
    def log_args(self, args):
        self.tbwriter.add_text(
            "args", json.dumps(args.__dict__, separators=(",\n", " : "), sort_keys=True)
        )
        self.tbwriter.file_writer.flush()
